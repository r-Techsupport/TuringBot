...
const channelLogging = new util.RootModule('logging', 'Manage discord channel and thread logging', [])
...
channelLogging.onInitialize(async () => {
  ...
  mBuffer.onWrite(async () => {
    ...
    const logChannel = util.client.channels.cache.get(logChannelID) as TextChannel
...
const populate = new util.SubModule(
  ...
  async (args, msg) => {
    ...
    const channels = util.guild.channels.cache.filter((ch: any) => ch.type === 0)
    ...
    const botResponse = await msg.reply({
      embeds: [util.embed.infoEmbed('Select channels to exclude from logging:')],
    ...
    /**
         ...
         */
    async function generateChannelsAndPopulateConfig (interaction: StringSelectMenuInteraction | ButtonInteraction): Promise<void> {
      ...
      if (unloggedChannels.length > 0) {
        ...
        await interaction.followUp({
          embeds: [
            util.embed.infoEmbed('No new channels need generation, moving onto config updates...')
          ]
        })
      }

      await interaction.followUp({
        embeds: [util.embed.infoEmbed('Generating and applying the correct config options...')]
      })
      ...
      interaction.followUp({ embeds: [util.embed.successEmbed('Config updated and logging deployed.')] })
    }
    ...
    continueButtonListener.on('collect', async (interaction: ButtonInteraction) => {
      await interaction.update({
        embeds: [util.embed.successEmbed('Continuing without modifying blacklist...')],
        components: []
      })
