...
import { APIEmbedField, Client, Events, GatewayIntentBits, APIEmbed, Guild } from 'discord.js'
    ...
    if (file.isDirectory()) {
      ...
      for (const subFile of subDirectory) {
        if (subFile.startsWith(file.name)) {
          await importModulesFromFile('../modules/' + file.name + '/' + subFile)
          break
        }
      }
    } else {
...
function listen (): void {
  client.on(Events.MessageCreate, async (message) => {
    ...
    if (currentModule.submodules.length === 0) {
      ...
      void message.reply({ embeds: [generateHelpMessageForModule(currentModule, commandUsed.join(' '))] })
    }
  })
}
