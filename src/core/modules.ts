...
export class Dependency {
  ...
  /**
     ...
     *
     */
  constructor (public name: string, private readonly attemptResolution: () => Promise<any>) {}
      ...
      eventLogger.logEvent(
        {
          ...
          location: 'core',
          description:
                        `Failed to resolve dependency ${this.name} due to error ${(err as Error).name}, ` +
  ...
  fetchValue (): NonNullable<any> {
    if (this.failed() || !this.resolutionAttempted()) {
      throw new Error(`Attempt made to access dependency "${this.name}" that was not resolved`)
    }
